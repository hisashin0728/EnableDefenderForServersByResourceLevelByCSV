...
Start-Transcript ".\DfSPP_Enabled_$filename.log"

# Generate Screen for checking
Write-Host "----------------------------------------------------------------------------"
Write-Host "[Private Preview] " -ForegroundColor Red  -NoNewline; Write-Host "This Powershell script is trying to enable Defender for Servers P1 as Resource Level (CSV bulk-mode)"
Write-Host "----------------------------------------------------------------------------"
...
foreach ($line in $csv){
    ...
    # Put RESTAPI
    $w = Invoke-WebRequest -Uri $restUri -InFile enable.json -Headers $authHeader -Method Put
